...
%%% run this
Data = xlsread('AllAggTensors.xlsx');
%% Choosing which tensors to plot
% 1 = Voigt, 2 = Reuss, 3 = VRH (only VRH for whole rock)
%%%William Note 9/14/2020 - make sure you change the VRH Number if we wish
%%%to look at a mineral class and not the whole rock - otherwise it will
%%%not execute
VRHTitle = {'Voigt';'Reuss';'VRH'};
VRHNum = 1;
% Choose phase (0 = whole rock, 1 = quartz, 2 = plagioclase,...)
MinNum = 1;

% change this to choose your sample
%%% William Note 9/14/2020 - Samples are listed under the sample column in
%%% AllAggTensors.xslx - there are 13 samples 
SampleN = 1;
FigName = ['Sample ',num2str(SampleN),' Phase = ',num2str(MinNum),' VRH = ',num2str(VRHNum)];


%%% William Note 9/14/2020 - This section appears to go through the columns
%%% and find the parameters as specified here. There are a few obvious
%%% ones, such as finding the minimum and the tensors, i do wonder about
%%% the specific outputs - we can experiment with this in matlab i believe
Min = find(Data(:,2) == MinNum);

VRH = find(Data(Min,4)==VRHNum);
Samps = find(Data(Min(VRH),1) == SampleN); 
Tensors = Data(Min(VRH(Samps)),5:25);
Rhos = Data(Min(VRH(Samps)),3);
Snum = Data(Min(VRH(Samps)),1);
Samples = num2str(Snum);
%%
%%% William Note - 9/14/2020 - This is just us selecting the tensors from
%%% the data file
for s = 1:length(Rhos)
...
%subplot(2,3,3)
x = contourf(Xtest,Ytest,Test(Xtest,Ytest),10,'EdgeColor','none');
...
plotlytest = plotlyfig(gcf)
plotlytest.PlotOptions.FileName = 'Quiver Plot';
plotlytest.PlotOptions.FileOpt = 'overwrite';
